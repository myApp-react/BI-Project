...
class Member extends PureComponent {
  render() {
    ...
    const { TotalCustomerCount, SequentialValue, CustPercentAmt, CustConsumeAmt, TotalCustAddCount } = memberData;
      ...
      >
        <div key="amache" className={'animated bounceInRight' + ' ' +styles.div1_text + ' ' + styles.four_corner_border}>
          <div className={styles['flex-left-text']}>
            <Title>会员</Title>
            {formaterVal(TotalCustAddCount, 0, 0, "人", "万人", { fontSize: 28,color: '#fff' }, {width: '100%'})}
            <IntroText>
              <ItemTitle>会员总数</ItemTitle>
              <Result>
                {formaterVal(TotalCustomerCount, 0, 0, "人", "万人", { fontSize: 16,color: '#fff' }, null)}
              </Result>
            </IntroText>
            <IntroText>
          ...
          <div className={styles['flex-1']}>
            <Recharts color={["#0fecf2"]}>
              ...
              <Grid left="4%" top="30%" bottom="10%" right="11%" containLabel={true} />
