...
/**重新渲染echarts tooltips*/
export function renderTooltips(params) {
  let basicMaker = '';
  let totalMaker = 0;
  ...
  params.forEach(_ => {
    if(_.seriesName === '会员销售' || _.seriesName === '非会员销售'){
      basicMaker += `${_.marker} ${_.seriesName}: ${numeral(_.data).format('0,0.00')} <br>`;
      totalMaker += Number(_.data);
    }else {
      if(_.seriesName === '客流'){
        basicMaker += `${_.marker} ${_.seriesName}: ${numeral(_.data).format('0,0')} <br>`;
      }else {
        basicMaker += `${_.marker} ${_.seriesName}: ${numeral(_.data).format('0,0.00')} <br>`;
      }
    }
  })
  return `${timeMaker} ${basicMaker} ${spanMaker} 总销售额：${numeral(totalMaker).format('0,0.00')}`
}

/**计算更新时间*/
export function getDateTime(lastLiving, configTime) {
  if(lastLiving.length === 0){
    // console.log(1)
    const Times = getSession("initTimes");
    let initTimes = '';
    if(Times){
      // console.log(2)
      initTimes = {
      ...
      initTimes = {
        StartTime: moment().subtract(Number(configTime), "minute").format("YYYY-MM-DD HH:mm:ss"),
        EndTime: moment().format("YYYY-MM-DD HH:mm:ss"),
        status: 1,
      }
      setSession("initTimes", initTimes)
    }
    return initTimes
  }else {
    // console.log(3)
    clearOneSession("initTimes");
    return {
      StartTime: moment(new Date(lastLiving.BillTime)).add(1, "seconds").format("YYYY-MM-DD HH:mm:ss"),
      EndTime: moment().format("YYYY-MM-DD HH:mm:ss"),
      status: 0,
    };
  }
